...
const Tutorials = () => {
  ...
  const handleEdit = (id) => {
    const itemToEdit = tutorials.find(item => item.id === id);
  ...
  const handleFormSubmit = (e) => {
    ...
    axios.put(`${apiUrl}/adminform/${currentEdit.id}`, currentEdit)
      .then(() => {
        setTutorials(tutorials.map(item => (item.id === currentEdit.id ? currentEdit : item)));
        handleModalClose();
      })
    ...
    <div className="flex flex-wrap">
      {tutorials.map(tutorial => (
        <div key={tutorial.id} className="max-w-sm rounded overflow-hidden shadow-lg bg-white m-4">
          <img className="w-full h-64 object-cover" src={tutorial.imageUrl} alt={`${tutorial.name} image`} />
